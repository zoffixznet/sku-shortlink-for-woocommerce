...
class SKU_Shortlink_For_WooCommerce_Settings extends WC_Settings_Page {
    ...
    public function all_settings( $settings, $current_section ) {
        if($this->id == $current_section){
            ...
            $settings[] = array(
                'name'     => __( 'URL Type', SKU_SF_WC_TEXT_DOMAIN ),
            ...
            $settings[] = array(
                'name'     => __( 'Custom Link', SKU_SF_WC_TEXT_DOMAIN ),
                'id'       => SKU_SF_WC_SLUG.'_custom_link',
                'type'     => 'text',
                'desc'     => __( 'Use <code>%sku%</code> to replace the value with sku code ', SKU_SF_WC_TEXT_DOMAIN ),
            );          
            ...
            $settings[] =   array('title' =>'', 
                                  'type' => 'title', 
                                  'desc' => __( 'Your Url Looks Like :', SKU_SF_WC_TEXT_DOMAIN ).' <code> <span id="sku_url">'.trailingslashit(home_url()).'<span id="sku_url_adds"></span> </span> </code> ', 'id' => 'pricing_options' );
